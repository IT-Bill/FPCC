...
def handler(signum, frame):
    raise TimeoutError("Execution exceeded the timeout.")
    ...
    def readAndCalculate(self):
        ...
        for line in self.rawData:
            ...
            if line.startswith("Function Index"):
                ...
                if flag_count > 1:
                    rank_res = []
                    for i,j,k in zip(temp_err,temp_inp,temp_cte):
                        rank_res.append([i,j,k])
                    ...
                    for i in rank_res:
                        ...
                        temp_inp.append(i[1])
                        temp_cte.append(i[2])
                    temp_res.append(curIndex-1)
                    temp_res.append(idx_time)
                    temp_res.append(temp_err)
                    temp_res.append(temp_inp)
                    temp_res.append(res_time)
                    temp_res.append(temp_cte)
                    temp_res.append(temp_orl)
                    temp_res.append(temp_out)
                    idx_res.append(temp_res)
                    res_time = 0.0
                    temp_res = []
                    temp_err = []
                    temp_inp = []
            ...
            if line.startswith("Data:"):
                ...
                try:
                    ...
                    temp_orl.append(oracleVal)
                    temp_out.append(outputY)
                    temp_err.append(relativeErr)
                    temp_inp.append(inputX)
                except TimeoutError:
                    print("TimeOut")
                et_time = time.time()-st_time
                idx_time = idx_time + et_time
                res_time = res_time + et_time
