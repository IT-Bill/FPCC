...
long getArg = 0;
long branchIns = 0;
double GLOBAL_BITSERROR;//*********updata*********
double sum_error = 0.0;//*********updata*********
int sum_count = 0;//*********updata*********
...
//double sum_error = 0.0;//*********updata*********
//int sum_count = 0;//*********updata*********
int max_bitserror = 0.0;
int min_bitserror = 100.0;
typedef struct Node{
  double bits_error;
  struct Node * next;
} Node;
...
void m_get_error(smem_entry *real, double computedVal){
  ...
  */
  ...
  sum_error += bitsError;
  sum_count ++;
  push(&s_err, bitsError);
  //fprintf(m_errfile, "%.2f\n",s_err.top->bits_error);
  fprintf(m_errfile, "%.2f\n",bitsError);
  if(bitsError > max_bitserror){
    max_bitserror = bitsError;
  }
  if(bitsError < min_bitserror){
    min_bitserror = bitsError;
  }
...
// change to calculate chain condition number
void handle_math_d_long(fp_op opCode, double lhs, smem_entry *op, 
		   double computedResd, smem_entry *res,
		   unsigned int linenumber, unsigned long long int ts) {
  double cond1 = 0.0;
  double lfit = 0.0;
  ...
  switch(opCode){
    ...
    case TAN:
      if(std::isinf(cond1)){
          cond1 = lfit * fabs(lhs / (sin(lhs+lfit*doubleULP(lhs)) * cos(lhs+lfit*doubleULP(lhs))));
      }else{
    ...
    case TANH:
      if(computedResd == 0.0){
          cond1 = lfit * fabs(lhs/doubleULP(lhs));
      }else{
          cond1 = lfit * fabs(lhs*(1-computedResd)/(computedResd*computedResd));
      }
      break;
    case SIN:
      if(computedResd == 0.0){
    ...
    case ACOSH:
      ...
      break;
    case ATAN:
      // cond1 = fabs(x/((x*x+1)*ATANH))
      if(computedResd==0){
          cond1 = lfit*fabs(lhs/doubleULP(lhs));
      }else{
          cond1 = lfit * fabs(lhs/((lhs*lhs + 1.0) * tan(lhs)));
      }
      break;
    case ATANH:
      cond1 = cond1;
    ...
    case LOG:
      if(lhs==1.0){
          ...
          cond1 =lfit * fabs(1 / computedResd);
    ...
    case LOG1P:
      if(lhs==0.0){
          if(lfit==0){
              ...
              cond1 = lfit * fabs(lhs /((lhs+1)*log1p(lhs+lfit*doubleULP(lhs))));
          }
      }else{
          cond1 =lfit * fabs(lhs /(lhs+1)*computedResd);
...
extern "C" void fpccsan_mpfr_fmod2(smem_entry* op1Idx, double op1d, 
  ...
  res->error = op1Idx->error + op2Idx->error;
  res->computed = computedRes;
  res->timestamp = ts;
#ifdef TRACING
  res->lhs = op1Idx; 
...
extern "C" void fpccsan_mpfr_atan22(smem_entry* op1Idx, double op1d, 
  ...
  if(computedRes==0){
      res->error = (op1Idx->error*op1d*op1d + op2Idx->error*op2d*op2d)/doubleULP(std::max(op1d,op2d));
  }else{
      res->error = (op1Idx->error*op1d*op1d + op2Idx->error*op2d*op2d)/((op1d*op1d+op2d*op2d)*computedRes);
  }
...
extern "C" void fpccsan_mpfr_powf2(smem_entry* op1Idx, float op1d, 
  ...
  double lhs = op1d;
  double rhs = op2d;
  double lfit = op1Idx->error;
  double rfit = op2Idx->error;
  if(computedRes==0){
      if(std::pow(lhs+lfit*doubleULP(lhs),rhs+rfit*doubleULP(rhs))==0){
          res->error = 1.0;
      }else{
          res->error=lfit * fabs(rhs) + rfit * fabs(rhs * log(lhs));
      }
  }else{
    res->error=lfit * fabs(rhs) + rfit * fabs(rhs * log(lhs));
  }
