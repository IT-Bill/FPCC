...
uint64_t fpUtil::rand64() {
    ...
    static std::mt19937_64 mt_generator_64(time(0));
...
uint32_t fpUtil::rand32() {
    ...
    static std::mt19937 mt_generator_32(time(0));
...
double fpUtil::disCalculation(uint64_t opcode, double lhs, double rhs, double ret, double lfit, double rfit) {
...
double fpUtil::reduceDis(double get_distance){
        double dis_exp = (double)fpUtil::getDoubleExpo(get_distance);
        double dis_frac = fpUtil::i64ToDouble(fpUtil::getDoubleFrac(get_distance));
        return dis_exp + dis_frac;
}
